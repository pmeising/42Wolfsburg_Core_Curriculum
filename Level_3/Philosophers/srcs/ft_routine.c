...
/*   Updated: 2023/01/05 11:35:33 by pmeising         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_helper_4(t_philos *philosopher)
{
	...
	time = ft_get_time();
	printf("%ld: %d has taken fork %d, %p.\n", (time - philosopher->start_time),
		philosopher->id, philosopher->left_fork->id, &philosopher->left_fork->id);
	printf("%ld: %d has taken fork %d, %p.\n", (time - philosopher->start_time),
		philosopher->id, philosopher->right_fork->id, &philosopher->right_fork->id);
